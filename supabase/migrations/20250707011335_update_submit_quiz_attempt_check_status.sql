-- Update fungsi submit_quiz_attempt untuk memeriksa status quiz
...
BEGIN
    ...
    -- Check if quiz is open for attempts
    SELECT status INTO v_quiz_status FROM public.quizzes WHERE id = p_quiz_id;
    
    IF v_quiz_status != 'open' THEN
        RAISE EXCEPTION 'Quiz is not available for submission. Current status: %', v_quiz_status;
    END IF;
