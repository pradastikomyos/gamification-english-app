CREATE OR REPLACE FUNCTION public.get_quiz_review_details(p_quiz_id uuid)
 RETURNS TABLE(quiz_title text, final_score numeric, base_score numeric, bonus_points numeric, time_taken_seconds integer, submitted_at timestamp with time zone, results_breakdown jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
    ...
    SELECT
        ...
        v_attempt.created_at AS submitted_at, -- Mengubah created_at menjadi submitted_at
        v_questions;
END;
$function$
