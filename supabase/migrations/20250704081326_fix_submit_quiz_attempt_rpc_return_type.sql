...
CREATE OR REPLACE FUNCTION public.submit_quiz_attempt(p_quiz_id uuid, p_student_answers jsonb, p_time_taken_seconds integer)
 RETURNS TABLE(score numeric, base_score numeric, bonus_points numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
...
BEGIN
    ...
    v_score := v_base_score + v_bonus_points;

    -- Insert into quiz_attempts
    INSERT INTO public.quiz_attempts (quiz_id, student_id, score, base_score, bonus_points, time_taken_seconds, completed_at, answers)
    VALUES (p_quiz_id, v_student_id, v_score, v_base_score, v_bonus_points, p_time_taken_seconds, NOW(), p_student_answers)
    ...
    RETURN QUERY SELECT v_score, v_base_score, v_bonus_points;
END;
$function$
