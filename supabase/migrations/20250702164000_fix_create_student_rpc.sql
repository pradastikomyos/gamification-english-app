...
BEGIN
  ...
  -- Insert into the public users/students table
  INSERT INTO public.users (id, name, email, role, student_id)
  VALUES (new_user_id, p_name, p_email, 'student', p_student_id);

  -- Return the details including the temporary password
  RETURN QUERY
  SELECT u.id, u.name, u.email, u.student_id, temp_password
  FROM public.users u
